...
print_hex_done:
  ...
  call print
  call post_process_start
  popa              ; pop the initial register values back from the stack
  ret               ; return the function

post_process_start:
    pusha
    mov bx, CONST       ;grab the pointer to the data
    mov si, HEX_OUT

post_process_loop:
    mov al, [bx]        ;get the current char from our pointer position
    add bx, 1           ;keep incrementing si until we see a null char
    cmp al, '-'
    je  post_process_done          ;end if the string is done
    mov [si], al           ;keep looping
    add si, 1
    jmp post_process_loop
post_process_done:
    popa
    ret
